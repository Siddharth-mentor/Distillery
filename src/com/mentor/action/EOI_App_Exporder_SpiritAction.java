...
import com.mentor.impl.EOI_App_Exporder_SpiritImpl;
import com.mentor.utility.Constants;

public class EOI_App_Exporder_SpiritAction {
	...
	public void setUpload_puc_certificate(String upload_puc_certificate) {
		...
		EOI_App_Exporder_SpiritImpl  impl = new EOI_App_Exporder_SpiritImpl();
		   ...
		   long	count_qty = 0;
			...
			if (validate_input) {
					...
					for (int i = 0; i < this.getBrand_detaiil_list().size(); i++) {
						EOI_App_Exporder_SpiritDT 
					dt = (EOI_App_Exporder_SpiritDT) this.getBrand_detaiil_list().get(i);
					//count_qty = count_qty + (int)dt.getRequested_qty();
					if(dt.getQty_bl() < dt.getRequested_bl() && dt.getQty_al() < dt.getRequested_al()){
						this.validate_input = false;
						FacesContext.getCurrentInstance().addMessage(null,
								new FacesMessage("Please Enter Requested Qty less than or equal to Quantity BL && AL"));
						break;
		...
		public void pdf3() {
			...
			try {
				...
				String mypath = Constants.JBOSS_SERVER_PATH
						...
						+ "ExciseUp" + File.separator + "ExportOutsideIndia"+ File.separator + "SPIRIT"+ File.separator + "APPLICATION";
				System.out.println(" ====mypath======"+mypath);
				InputStream io = new FileInputStream(this.upload1);

				String recordFile = "performa_certificate_"+n+"-"+distillery_id+ ".pdf";
						System.out.println("====111111---recordFilerecordFile==="+recordFile);
	...
	public void reset(){
		...
		EOI_App_Exporder_SpiritDT dt = (EOI_App_Exporder_SpiritDT) 
		...
		this.detail_list.remove(dt);
			...
			public ArrayList getDetail_list() throws ArithmeticException 
